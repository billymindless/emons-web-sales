---
name: 동 단위 상권 퍼포먼스 맵
overview: CRM 매출 데이터(app_customers/app_orders)를 카카오 로컬 API로 행정동 단위까지 지오코딩하고, 행정안전부 공공데이터(행정동별 인구·세대현황)와 결합해 "시장 침투율 × 타겟 밀집도" 2x2 매트릭스로 상권을 A/B/C/D 4그룹으로 분류, Plotly Mapbox choropleth+버블맵으로 시각화하는 신규 대시보드 기능의 마스터 플랜.
todos:
...
- 타겟 밀집도 = (해당 행정동 3040 인구수 / 해당 행정동 총 인구수) × 100 (세대수 기준 3040 세대 추출이 불가하므로 인구 비율로 우회)
