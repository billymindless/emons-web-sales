...
- 매출 엑셀 임포트 UI 완성 — [legacy_import_service.py](legacy_import_service.py), [app.py](app.py) `_render_legacy_order_bulk_import`
- 마이그레이션 파일 [SUPABASE_APP_ORDERS_IMPORT_SOURCE.sql](SUPABASE_APP_ORDERS_IMPORT_SOURCE.sql) — 사용자가 Supabase 에서 실행 필요
- 매출 임포트 시 매장별 `phone1(digits)` 기준 재구매 통합, `balance_status='완납'`, `import_source='엑셀_과거이력'` 태그, (customer_id, order_date, cost_price) 지문 기반 중복 스킵.
