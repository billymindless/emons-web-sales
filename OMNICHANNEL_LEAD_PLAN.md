...
> 작성일: 2026-06-08  
> 프로젝트: emons-web-sales (momo 시스템)  
> 스택: Supabase · FastAPI (api.py) · Streamlit (app.py) · Solapi · 채널톡
...
채널톡은 익명 유저 ID(`w_anom_1234`)가 세션마다 바뀌는 구조적 한계가 있다. 이 시스템은 **전화번호 하나만 확보되면 과거 모든 이력(카카오톡, 오프라인 방문, 웹챗)을 Supabase에서 병합**하여 상담원에게 즉시 시각화한다. 채널톡의 세션 유실 한계를 우리 백엔드 아키텍처로 방어하는 구조다.
...
| 필드 | 설명 |
|---|---|
| `customer_phone` | 전화번호 — 모든 채널 이력을 병합하는 단일 식별자 |
| `channel` | `채널톡_웹챗` / `카카오톡` / `오프라인_메모` / `전화_통화` |
| `chat_id` | 채널톡 chatId — `chat.closed` 웹훅으로 대화 전문 수집 시 사용 |
| `summary` | 상담 요약 (앞 200자 자동 추출 또는 직원 직접 입력) |
| `full_text` | 대화 전문 전체 텍스트 |
...
| 필드 | 타입 | 설명 |
|---|---|---|
| `lead_source` | TEXT (CHECK) | 유입 경로 3가지로 엄격히 통제 |
| `lead_stage` | TEXT (CHECK) | 리드 온도 5단계 파이프라인 |
| `assigned_employee_id` | BIGINT FK | KPI 산출의 기본키 — app_users 참조 |
| `next_contact_date` | DATE | 영업 직원 Follow-up 데드라인 |
| `followup_done` | BOOLEAN | 기한 내 사후 관리 완료 여부 |
| `nurturing_step` | INT | 0=미발송 / 1=T+0 완료 / 2=T+N 완료 |
| `next_nurture_at` | TIMESTAMPTZ | 다음 예약 발송 시각 (cron 기준) |
| `ct_synced` | BOOLEAN | 채널톡 v5/users upsert 완료 여부 |
| `converted_at` | TIMESTAMPTZ | 계약 완료 시각 (자동 기록) |
| `converted_order_id` | BIGINT FK | 연결된 주문 ID — app_orders 참조 |
| `revenue_amount` | NUMERIC | 계약 금액 — KPI 매출 집계 기준 |
...
| 유입 경로 | 1순위 | 2순위 | 3순위(폴백) |
|---|---|---|---|
| 오프라인 방문 | MMS (명함+사진) | — | SMS |
| 전화 문의 | 친구톡 | 알림톡 | SMS |
| 온라인 채널톡 | 친구톡 | 알림톡 | SMS |
...
| 방법 | 설명 |
|---|---|
| 서포트봇 폼 | 웹챗 시작 시 "재상담 내역 조회를 위해 전화번호를 입력해 주세요" 폼 표시 |
| 상담원 수동 입력 | 대화 중 번호를 물어보고 우측 프로필 `mobileNumber` 란에 직접 입력 |
| 기존 유저 | 이전에 채널톡에 번호가 등록된 유저는 자동 연동 |

세 가지 방법 모두 채널톡 유저 프로필의 `mobileNumber`를 채우며, 이후 Snippet 호출 시 동일하게 활용된다.

### 2단계 PULL — Snippet 엔드포인트 고도화

기존 `/channel-talk/custom-tab`에 익명 분기 + 통합 이력 조회 추가:
...
async def handle_custom_tab(payload: dict):
    ...
    # app_chat_history 통합 이력 조회 (최신 3건)
...
### 3단계 PUSH — `chat.closed` 웹훅 대화 전문 자동 아카이빙
...
async def handle_ct_webhook(payload: dict):
    if payload.get("event") != "chat.closed":
        return {"ok": True}
...
| 타이밍 | 발송 내용 | 수단 |
|---|---|---|
| T+0 즉시 | "오늘 방문해 주셔서 감사합니다. 담당자 OOO입니다. 보셨던 [제품명] 사진을 보내드립니다." + 매장/제품 사진 첨부 | MMS |
| T+3일 | "사이즈나 색상 결정에 어려움이 있으신가요? 채널톡으로 문의주시면 바로 답변드리겠습니다. [채널톡 링크]" | SMS 또는 친구톡 |
...
GET /run-lead-care
  ...
  → nurturing_step += 1, next_nurture_at 갱신 (또는 NULL로 종료)
```
...
`api.py`의 `_build_ct_response()`를 확장 — `app_leads` 테이블도 조회하여 가망고객 정보 노출:
...
`app.py`의 **고객 상세 화면**에 "상담 이력" 섹션을 추가한다.  
`app_chat_history`를 `customer_phone`으로 조회하여 채널톡, 카카오톡, 오프라인 메모, 전화 통화를 **한 화면에 통합 표시**.
...
| 항목 | 구현 방법 |
|---|---|
| 데이터 조회 | `app_chat_history WHERE customer_phone = 고객 phone1` |
| 채널 시각적 구분 | 채널별 색상/아이콘 (채널톡=파랑, 카카오=노랑, 오프라인=초록, 전화=회색) |
| 전체 대화 펼치기 | `st.expander`로 `full_text` 숨김/표시 |
| 메모 직접 추가 | `[+ 메모 추가]` 버튼 → 폼 → `app_chat_history` INSERT (전화 상담/오프라인 메모용) |
| 리드 연결 표시 | 동일 전화번호의 `app_leads` 리드 단계도 함께 노출 |
...
```
채널 선택: [전화_통화 ▼]  (전화_통화 / 오프라인_메모 / 카카오톡)
상담 내용: [직접 입력...]
담당자:    [자동: 로그인 직원]
...
저장 시 `app_chat_history`에 INSERT → 채널톡 Snippet에서도 다음 재상담 시 자동 노출.

---

## 6단계: KPI 대시보드 (`app.py` 신규 탭)

메인 탭 목록에 **"세일즈 퍼포먼스"** 탭 추가. `app_leads` 집계 쿼리 기반.
...
**의미:** 담당한 가망고객 중 실제 계약으로 이어진 비율. 영업력의 핵심 지표.
...
**의미:** 해당 직원이 발굴·클로징한 리드로부터 발생한 총 매출액. 인센티브 산출 기준.  
...
**의미:** 첫 접촉부터 계약까지 걸린 평균 일수. 영업 사이클 단축 여부를 직원별·채널별로 비교.
...
**기한 내 처리 기준:** `next_contact_date` + 1일 이내에 `contact_memo`가 입력된 건

**의미:** 자동 문자 발송 외에, 직원이 직접 전화를 걸어 클로징을 시도했는지 평가.
...
**의미:** 매장 Walk-in 고객 중 실제로 번호를 확보하여 momo에 등록한 비율.  
...
| `api.py` | `/channel-talk/custom-tab` 익명 분기 + 이력 조회, `/channel-talk/webhook` chat.closed 아카이빙, `_ct_upsert_user()`, `/run-lead-care` |
| `app.py` (고객 상세) | 고객 상세 화면에 "상담 이력" 탭/섹션 추가 — `app_chat_history` 조회 + 메모 직접 추가 |
| `lead_manager.py` (신규) | `register_lead()`, `_auto_close_lead()`, 넛징 시나리오 발송 함수 |
| `crm_automation.py` | 가망고객 등록 UI 섹션 (전화/오프라인) |
| `app.py` | 매출 등록 시 `_auto_close_lead()` 호출 + 세일즈 퍼포먼스 KPI 탭 신설 |
| `render.yaml` | `CHANNEL_TALK_ACCESS_KEY`, `CHANNEL_TALK_ACCESS_SECRET` 환경변수 추가 |
...
| 방문자 수 입력 UI | KPI 3 분모 — `app_store_daily_visits` 테이블 신규 또는 직원 수동 입력 |

---

## 구현 순서 (Todo)

1. **Step 0** `SUPABASE_APP_CHAT_HISTORY.sql` 생성 및 Supabase 실행
2. **Step 1** `SUPABASE_APP_LEADS.sql` 생성 (전환 추적 필드 포함) 및 Supabase 실행
3. **Step 2** `solapi_sender.py`에 `send_sms()` / `send_mms()` 추가
4. **Step 3** `api.py` `/channel-talk/custom-tab` 익명 분기 + `app_chat_history` 통합 이력 조회
5. **Step 4** `api.py` `/channel-talk/webhook` — `chat.closed` 이벤트 수신 및 대화 전문 아카이빙
6. **Step 5** `api.py` 채널톡 v5/users upsert + 온라인 채널톡 리드 자동 등록 웹훅 추가
7. **Step 6** `crm_automation.py`에 옴니채널 리드 등록 UI 및 `register_lead()` 구현
8. **Step 7** 유입 경로별 넛징 시나리오 발송 로직 구현 (T+0 즉시, T+N일 예약)
9. **Step 8** `app.py` 매출 등록 시 `_auto_close_lead()` 자동 호출 추가
10. **Step 8.5** `app.py` 고객 상세 화면에 "상담 이력" 탭 추가 (전채널 통합 뷰 + 메모 직접 추가)
11. **Step 9** `app.py`에 세일즈 퍼포먼스 KPI 탭 신설 (5대 지표)
12. **Step 10** `api.py` `/run-lead-care` 엔드포인트로 예약 발송 실행기 구현
