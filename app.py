...
# ========== Flash 메시지 (rerun 후에도 한 번 표시) ==========
def flash(message: str, level: str = "success"):
    """st.rerun() 직전에 호출하면, 다음 페이지 로드 시 메시지가 한 번 표시됩니다.
    저장/수정 완료 메시지가 rerun으로 사라지는 문제를 방지합니다."""
    msgs = st.session_state.get("_flash_msgs", [])
    msgs.append({"level": level, "message": str(message)})
    ...
    with st.form("new_store_form"):
        ...
        if submitted and store_name and store_name.strip():
            ...
            try:
                ...
                clear_data_cache()
                flash(f"매장 '{store_name}'이(가) 생성되었습니다. DB: {db_filename}")
                st.rerun()
            except Exception as e:
                err_str = str(e).lower()
                    ...
                    if use_supabase:
                        ...
                        try:
                            if existing:
                                ...
                                if supabase_already_exists:
                                    flash("이 이메일은 Supabase에 이미 있어 앱 권한만 부여했습니다. 기존 비밀번호로 로그인할 수 있습니다.")
                                else:
                                    flash("직원 계정이 생성되었습니다. 해당 이메일과 초기 비밀번호로 로그인할 수 있습니다.")
                        except Exception as e:
                        ...
                        try:
                            ...
                            if existing:
                                ...
                                conn.commit()
                                flash("이미 Supabase에 있는 이메일입니다. 직원 정보(이름, 권한, 배정 매장)만 반영했습니다. 기존 비밀번호로 로그인할 수 있습니다.")
                            else:
                                conn.execute(
                                    """
                            ...
                            try:
                                if use_supabase:
                                    ...
                                    try:
                                        ...
                                        conn.close()
                                flash(f"직원 정보가 저장되었습니다. 기본 매장: {edit_primary_store}")
                                st.rerun()
                            except Exception as e:
                        ...
                        if st.form_submit_button("이메일 변경"):
                            ...
                            if not new_email_clean or "@" not in new_email_clean:
                                ...
                                if email_errs:
                                    ...
                                    clear_data_cache()
                                    flash(f"이메일이 {new_email_clean} 으로 변경되었습니다.")
                                    st.rerun()
                                ...
                                if not target_email:
                                    ...
                                    try:
                                        ...
                                        if auth_uid:
                                            ...
                                            clear_data_cache()
                                            flash("비밀번호가 변경되었습니다. 해당 직원은 다음 로그인부터 새 비밀번호를 사용합니다.")
                                            st.rerun()
                                        else:
                                            ...
                                            try:
                                                ...
                                                if created_uid:
                                                    clear_data_cache()
                                                    flash("Supabase Auth 계정이 없어 새로 생성한 뒤 비밀번호를 설정했습니다. 해당 직원은 즉시 로그인할 수 있습니다.")
                                                    st.rerun()
                                                else:
                                                ...
                                                if "already been registered" in ce_msg.lower() or "already registered" in ce_msg.lower():
                                                    try:
                                                        ...
                                                        if auth_uid_retry:
                                                            ...
                                                            clear_data_cache()
                                                            flash("기존 Supabase Auth 계정을 찾아 비밀번호를 변경했습니다.")
                                                            st.rerun()
                                                        else:
                        ...
                        try:
                            if use_supabase:
                                ...
                                try:
                                    ...
                                    conn.close()
                            flash(f"배정 매장이 저장되었습니다. 기본 매장: {edit_primary_store2}")
                            st.rerun()
                        except Exception as e:
                            if not use_supabase:
                                try:
...
APP_FAQ_ITEMS: list[dict[str, str]] = [
    ...
    },
    {
        ...
        "body": (
            ...
            "한 번의 결제(예: 온누리상품권 100만원)를 여러 주문에 나누어 등록하는 기능입니다.\n\n"
            "**사용 위치**\n\n"
            "잔금 결제 화면에서 해당 고객의 잔금 있는 주문이 2건 이상이면 자동으로 "
            "「💳 복수 주문 분배 결제」 구역이 표시됩니다.\n\n"
            "**입력 순서**\n\n"
            "1. **실제 입금액 ★** — 고객이 실제로 납부한 **총 금액**을 입력합니다. "
            "이 금액과 아래 각 주문 배분 합계가 **일치해야** 등록 버튼이 정상 작동합니다.\n"
            "2. **결제 수단 / 결제 날짜** — 이번 결제에 사용한 수단과 날짜를 선택합니다.\n"
            "3. **주문별 배분 금액** — 각 주문에 나눌 금액을 직접 입력합니다. "
            "초기값은 각 주문의 잔금 전액으로 자동 채워집니다.\n"
            "4. **처리 사유** — 5자 이상 입력해야 등록됩니다 (예: '온누리 100만 분배').\n\n"
            "**자동 검증 내용**\n\n"
            "- 배분 합계와 실제 입금액이 다르면 화면에 **❌ 초과/부족 메시지**가 표시되고 등록이 차단됩니다.\n"
            "- 특정 주문에 배분한 금액이 해당 주문의 잔금보다 크면 **⚠️ 초과 경고**가 표시됩니다 "
